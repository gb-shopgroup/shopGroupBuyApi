...
@Data
public class LeaderGroupResponse {
    ...
    // 团购商品列表
    private List<LeaderGroupGoodsResponse> goods;

    public LeaderGroupResponse(){

    }

    public LeaderGroupResponse(GbGroupActivityInfo data){
    ...
    // 列表转换
    public static List<LeaderGroupResponse> getGroupResponseList(List<GbGroupActivityInfo> lists){

        List<LeaderGroupResponse> data = new ArrayList<>();
        for(GbGroupActivityInfo item : lists){
            data.add(new LeaderGroupResponse(item));
        }
        return data;
    }
